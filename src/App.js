  ...
  useEffect(() => {
    (async function getFswapBalance() {
      if (web3Provider !== null) {
        // const fswapContract = new web3Provider.eth.Contract(
        //   minABI,
        //   fswapAddress
        // );
        // if (account !== "") {
    ...
    (async function getBtaBalance() {
      if (web3Provider !== null) {
        if (account !== "") {
          const bal = await web3Provider.utils.fromWei(
            await web3Provider.eth.getBalance(account)
          );
