...
function Header({
    ...
    if (netid !== netID) {
      if (isMetaMask) {
        try {
          ...
          (() => toast.success("Network switched To CoreDao"))();
        } catch (switchError) {
          ...
          if (switchError.code === -32603) {
            try {
              ...
              (() => toast.success("CoreDao Chain Added"))();
            } catch (addError) {
              // handle "add" error
  ...
  async function getBothBalances(acct) {
    if (acct !== "") {
      const btabal = await window.web3.utils.fromWei(
        await window.web3.eth.getBalance(acct)
      );
  ...
  return (
    <nav
      className="navbar navbar-expand-md navbar-dark fixed-top navbar-index"
      id="index_nav"
    >
      <div className="container size-navbar" id="size">
        ...
        <button
          id="nav_btn"
          onClick={connected ? toggleAcctInfo : connectWallet}
          className="btn btn-outline-success btn-main align-right custom-center-btn"
        >
          <span>
            <FaWallet />
          ...
          {showAcctInfo && (
            <div
              className="userAcctInfo"
              onMouseEnter={showInfo}
              onMouseLeave={hideInfo}
            >
              <p className="currencyName">CORE</p>
              <p className="currencyAmt">{updatedBtaBal}</p>
              <p className="currencyName">MIGO</p>
